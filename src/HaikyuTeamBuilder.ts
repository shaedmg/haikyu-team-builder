...
export class HaikyuTeamBuilder {
  ...
  private async loadImageMapping(): Promise<void> {
    try {
      ...
      console.log(
        `Loaded image mapping for ${Object.keys(this.imageMapping).length} characters`
      );
    } catch (error) {
      console.warn('Error loading image mapping:', (error as Error).message);
      this.imageMapping = {};
    }
  }
  ...
  private getPlayerImageUrl(player: Character): string {
    ...
    if (this.imageMapping && player.id && this.imageMapping[player.id]) {
      return (this.imageMapping[player.id] as any).local_path;
    }
  ...
  private formatBonusText(bonusValue: string): string {
    ...
    if (complexMatch) {
      ...
      const result = `${points} ${
        window.languageManager ? 'puntos' : 'puntos'
      } ${percentage} ${
  ...
  private renderBonds(): void {
    ...
    const relevantBonds = this.bonds
      .filter((bond) => {
        // Only show bonds that have is_link_skill: true
        if (!(bond as any).is_link_skill) {
          return false;
        }
  ...
  private generateBondEffectHTML(bond: any, currentPlayerIds: number[] = []): string {
    // Simple implementation for TypeScript version
